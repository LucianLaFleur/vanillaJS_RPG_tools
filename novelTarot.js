...
function display8PointPlotter(cardsArr, intensityArr) {
    const outputDiv = document.getElementById("tarot-8Point-Plot");
    ...
    cardsArr.forEach((card, index) => {
        ...
        p2.textContent = `${intensityArr[index]} -- ${tarotDeckInfo[card]}`;
        p3.textContent = " ----------- "
...
function get24PtPlot(cardsArr, intensityArr){
    const outputDiv = document.getElementById("tarot-8Point-Plot");
    outputDiv.innerHTML = ""; // Clear any existing content
    cardsArr.forEach((card, index) => {
        // card can be used to access the dictionary being made, NYI
        ...
        const p3 = document.createElement("p");
        if (index == 0) {
            p.textContent = `1) Really Bad Day (the normalcy for this world and the character’s place in it are established) -  ` 
        } else if (index == 1){
            p.textContent = `2) Strange Thing Breaks The Mold (something upsets the usual pace of things, and we see how the world reacts) ` 
        } else if (index == 2){
            p.textContent = `3) escalating upset leads to an opportunity (the hero actively reacts to the change in pace and makes an active decision)` 
        } else if (index == 3){
            p.textContent = `4) Call to Adventure (Key figures / info about new world exposed)` 
        } else if (index == 4){
            p.textContent = `5) Out of depth moment (show progress with new world, but struggle with main theme)` 
        } else if (index == 5){
            p.textContent = `6) Antagonist’s Opening Gambit (just as the heroes are making progress, an opposing force ups the ante and reveals things are much tougher than before)` 
        } else if (index == 6){
            p.textContent = `7) Gathering power (usually, reach out to allies, avoid enemies.)` 
        } else if (index == 7){
            p.textContent = `8) Trials (fun and games, character development, and overcoming the main character’s weakness)` 
        } else if (index == 8){
            p.textContent = `9) Proving Growth (show progress towards primary goal and overcoming former weaknesses)` 
        } else if (index == 9){
            p.textContent = `10) Evil response (protagonists put on back foot by the antagonists’ new development)` 
        } else if (index == 10){
            p.textContent = `11) Planning reveals complications (trying to fix the new complications shows a new depth to the antagonist’s power)` 
        } else if (index == 11){
            p.textContent = `12) clarification of consequences and a demand for action (the heroes are faced with a severe decision about what to do about the full extent of the threat and its consequences.)` 
        } else if (index == 12){
            p.textContent = `13) reflection on primary motivation (question why they are still there)` 
        } else if (index == 13){
            p.textContent = `14) revised plans (preparing to carry out the results of the evaluation, secondary characters react to decision)` 
        } else if (index == 14){
            p.textContent = ` 15) In insightful glimpse (an important advancement towards the hero’s new desire is made, giving a glimpse of either hope or despair as the hero is rooted in, or disillusioned from, their new plans. *Climax of B-plot)` 
        } else if (index == 15){
            p.textContent = `16) direct action - the most logical, straightforward solutions to the antagonistic problem are attempted.` 
        } else if (index == 16){
            p.textContent = ` 17) antagonist’s counter - the villain reveals they are competent by thwarting the direct solution with a counter-play (might be foreshadowed before, but ignored because the main character doesn’t have influence).` 
        } else if (index == 17){
            p.textContent = `18) key revelation - (an important element is found, which can turn the tide of the central conflict *Resolves B-plot)` 
        } else if (index == 18){
            p.textContent = `19) severe setback pushes to the breaking point (the heroes are almost entirely defeated and face the edge of ultimate defeat)` 
        } else if (index == 19){
            p.textContent = `20) pep-talk/intervention helps to give hope (a risky idea offers some small chance of success)` 
        } else if (index == 20){
            p.textContent = `21) Calvary ride (a fearless charge towards the risk, wholeheartedly)` 
        } else if (index == 21){
            p.textContent = `22) The tower falls (ultimate devastation falls all around the heroes, and it appears that all is lost)` 
        } else if (index == 22){
            p.textContent = `23) Heroic moment (an epic feat allows for the hero to overcome even the worst of circumstances and defeat the ultimate evil)` 
        } else if (index == 23){
            p.textContent = `24) return and reflect (establish a new normal. show how things are different from their original life, reincorporate main theme to show character development. Show how the heroes’ actions changed everything)` 
        } 
    p2.textContent = `${intensityArr[index]} -- ${tarotDeckInfo[card]}`;
        p3.textContent = " ----------- "
        outputDiv.appendChild(p);
...
function displayCharacter(cardsArr, intensityArr) {
    const outputDiv = document.getElementById("tarot-character-outline");
    outputDiv.innerHTML = ""; // Clear any existing content
    cardsArr.forEach((card, index) => {
        // card can be used to access the dictionary of tarot info  being made, NYI
        const p = document.createElement("p");
        const p2 = document.createElement("p")
        if (index == 0) {
            p.textContent = `archetype pt 1 ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 1){
            p.textContent = `archetype pt 2 ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 2){
            p.textContent = `key desire ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 3){
            p.textContent = `likable trait ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 4){
            p.textContent = `competency ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 5){
            p.textContent = `personality 1 ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 6){
            p.textContent = `personality 2 ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 7){
            p.textContent = `Appearance ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 8){
            p.textContent = `Primary flaw ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 9){
            p.textContent = `Background ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 10){
            p.textContent = `Connections ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 11){
            p.textContent = `Near Future ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 12){
            p.textContent = `Character Arc Crux ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else {
            p.textContent = `Appearance motif ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        }
        outputDiv.appendChild(p);
    });
}

function makeInitShotgun(cardsArr, intensityArr) {
    const outputDiv = document.getElementById("init-shotgun-spread");
    outputDiv.innerHTML = ""; // Clear any existing content
    cardsArr.forEach((card, index) => {
        // card can be used to access the dictionary being made, NYI
        const p = document.createElement("p");
        const p2 = document.createElement("p");
        if (index == 0) {
            p.textContent = `Core theme : ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 1){
            p.textContent = `B-plot theme : ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 2){
            p.textContent = `Main Hero : ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 3){
            p.textContent = `Hero Desire : ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 4){
            p.textContent = `Main Villain : ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 5){
            p.textContent = `Villian Desire : ${intensityArr[index]} -- ${tarotDeckInfo[card]}`
        } else if (index == 6){
            p.textContent = `(develop 1 sentence summary:) Primary conflict on theme: : ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else {
            p.textContent = `Setting, nutshell : ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        }
        p2.textContent = " ----------- "
        outputDiv.appendChild(p);
        outputDiv.appendChild(p2);
...
function simpleFoilChar(cardsArr, intensityArr) {
    const outputDiv = document.getElementById("foil-chars");
    outputDiv.innerHTML = ""; // Clear any existing content
    cardsArr.forEach((card, index) => {
        // card can be used to access the dictionary being made, NYI
        const p = document.createElement("p");
        const p2 = document.createElement("p");
        if (index == 0) {
            p.textContent = `Char-A Archetype: ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 1){
            p.textContent = `Char-A Quirk: ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 2){
            p.textContent = `Char-A Desire: ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 3){
            p.textContent = `Char-B Archetype: ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 4){
            p.textContent = `Char-B Quirk: ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 5){
            p.textContent = `Char-B Desire: ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 6){
            p.textContent = `Why they're in conflict : ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } 
        p2.textContent = " ----------- "
        outputDiv.appendChild(p);
        outputDiv.appendChild(p2);
...
function clearDisplayedData(){
    const element1 = document.getElementById("tarot-8Point-Plot");
    const element2 = document.getElementById("tarot-character-outline");
    const element3 = document.getElementById("init-shotgun-spread");
    const element4 = document.getElementById("foil-chars");
    const allElements = [element1, element2, element3, element4]
    // clear 'em all
...
function quick3Cards(cardsArr, intensityArr) {
    const outputDiv = document.getElementById("tarot-8Point-Plot");
    outputDiv.innerHTML = ""; // Clear any existing content
    cardsArr.forEach((card, index) => {
        // card can be used to access the dictionary being made, NYI
        ...
        const p2 = document.createElement("p");
        if (index == 0) {
            p.textContent = `I. -  ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 1){
            p.textContent = `II. -  ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } else if (index == 2){
            p.textContent = `III.  -  ${intensityArr[index]} -- ${tarotDeckInfo[card]}` 
        } 
        p2.textContent = " ----------- "
        outputDiv.appendChild(p);
        outputDiv.appendChild(p2);
...
function makeAndSelectCardsOfLen(inputLength){
    let chosenCards = makeCards()
    let cardSubset = chosenCards.slice(0,inputLength)
    ...
    return [arrWithReversals, arrWithIntensity]
}

document.getElementById('dumRoll1').addEventListener('click', () => {
    // for the 8 point plot version
    eightChosenCards = makeAndSelectCardsOfLen(8)
    display8PointPlotter(eightChosenCards[0], eightChosenCards[1])
    console.log("--------------")
});

document.getElementById('dumRoll2').addEventListener('click', () => {
    fourteenChosenCards = makeAndSelectCardsOfLen(14)
    displayCharacter(fourteenChosenCards[0], fourteenChosenCards[1])
    console.log("--------------")
});

document.getElementById('dumRoll3').addEventListener('click', () => {
    eightChosenCards = makeAndSelectCardsOfLen(8)
    makeInitShotgun(eightChosenCards[0], eightChosenCards[1])
    console.log("--------------")
});

document.getElementById('dumRoll4').addEventListener('click', () => {
    sevenChosenCards = makeAndSelectCardsOfLen(7)
    simpleFoilChar(sevenChosenCards[0], sevenChosenCards[1])
    console.log("--------------")
});

document.getElementById('dumRoll5').addEventListener('click', () => {
    myChosenCards = makeAndSelectCardsOfLen(24)
    get24PtPlot(myChosenCards[0], myChosenCards[1])
    console.log("--------------")
});

document.getElementById('dumRoll6').addEventListener('click', () => {
    myChosenCards = makeAndSelectCardsOfLen(3)
    quick3Cards(myChosenCards[0], myChosenCards[1])
    console.log("--------------")
});

document.getElementById('clear-all-btn').addEventListener('click', () => {
    if (!confirm("Are you sure you want to proceed?")) {
        console.log("no action taken"); // Prevent the default button action
      } else {
        console.log("deletion done");
        clearDisplayedData()
      }
    console.log("-----Displayed Data Cleared-------")
});
